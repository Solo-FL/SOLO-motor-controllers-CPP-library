...
bool MCP2515::CANOpenTransmit(uint8_t _address ,uint16_t _object, uint8_t subIndex, uint8_t * _informatrionToSend, int &error)  
{
    (void) subIndex;
    uint16_t ID_Read ; 
    uint8_t  DLC_Read ;
    ...
    //@@ else 
    //@@ return some kind of error - yes!! you must return something
    error = SOLOMotorControllers::Error::generalError;
    return true;
}

bool MCP2515::CANOpenReceive(uint8_t _address ,uint16_t _object, uint8_t subIndex, uint8_t * _informatrionToSend, uint8_t * _informationReceived, int &error)
{
    (void) subIndex;
    uint16_t ID_Read ; 
    uint8_t  DLC_Read ;

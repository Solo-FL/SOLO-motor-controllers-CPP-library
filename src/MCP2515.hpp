...
#ifndef MCP2515_H       //Avoid loading MCP2515.h more than once
#define MCP2515_H
...
public:
    //Constructor
    MCP2515(uint16_t baudrate, uint8_t _chipSelectPin, long _countTimeout);
    ...
    bool SendPdoSync(int& error) { (void) error; return true; };
	bool SendPdoRtr(int destination, int& error) { (void) destination; (void) error; return true; };
	bool PDOTransmit(int destination, uint8_t* informationToSend, int& error) {
        (void) destination; (void) informationToSend; (void) error;
        return true;
    };
	bool PDOReceive(int source,  uint8_t* informationReceived, int& error) {
        (void) source; (void) informationReceived; (void) error;
        return true;
    };
    ...
    uint8_t getTransmitErrorCounter() override { return MCP2515_Read_TransmitErrorCounter(); };

    void Generic_Canbus_Read_MCP2515(uint16_t *_ID , uint8_t *_DLC, uint8_t *_Data)
    {
        *_ID = 0;
        *_DLC = 0;
        if((MCP2515_Read_RX_Status() & (1<<6))){
            MCP2515_Receive_Frame(MCP2515_RX_BUF::RX_BUFFER_0,_ID,_DLC,_Data);
        }
    }
    void Generic_Canbus_Write_MCP2515(uint16_t _ID, uint8_t _DLC, uint8_t *_Data, int &error)
    {
        MCP2515_Transmit_Frame(MCP2515_TX_BUF::TX_BUFFER_0, _ID, _DLC, _Data, error);
    }
